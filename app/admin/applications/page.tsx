...
// Simulated components for compilation completeness
function Navbar() {
  return <div className="w-full bg-white h-14 border-b border-slate-200" />;
}

const JOBS_API_URL = "http://localhost:5000/jobs";
const APPS_API_URL = "http://localhost:5000/applications";
...
export default function AdminApplications() {
  ...
  const getApplications = async () => {
    setLoading(true);
    try {
      // 1. Fetch raw application data from local json-server
      const appsResponse = await fetch(APPS_API_URL);
      if (!appsResponse.ok) throw new Error("Could not retrieve applications database.");
      const appsData = await appsResponse.json();

      if (appsData && appsData.length > 0) {
        // 2. Fetch job listing contextual lookup map
        const jobsResponse = await fetch(JOBS_API_URL);
        if (!jobsResponse.ok) throw new Error("Could not retrieve job lookup tables.");
        const jobsData: any[] = await jobsResponse.json();

        // 3. Replicate structural joins in memory & organize chronologically (newest first)
  ...
  const filteredApplications = applications.filter((app) => {
    const matchesName = app.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) || false;
    const matchesEmail = app.email?.toLowerCase().includes(searchTerm.toLowerCase()) || false;
